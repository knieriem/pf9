...
push(int fd, char *cmd, int input, Pushstate *ps)
{
	...
	fds[input? Stdin: Stdout] = dup(fd, -1);
	fds[Stderr] = dup(Stderr, -1);
	ps->kid = threadspawnl(fds, cmd, cmd, nil);
	if (ps->kid==-1)
		sysfatal("can't spawn %s: %r", cmd);
	ps->rfd = nfd;
	ps->open = 1;
	return nfd;
}
