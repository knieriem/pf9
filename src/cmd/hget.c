...
doftp(URL *u, URL *px, Range *r, Out *out, long mtime)
{
	...
	/* fork */
	arg = malloc(sizeof(Ftpxferarg));
	if (arg==nil) {
		close(data);
		return terminateftp(ctl, Error);
	}
	cwait = chancreate(sizeof(ulong), 0);
	arg->in = data;
	arg->out = out;
	arg->r = r;
	arg->cwait = cwait;
	proccreate(ftpxferproc, arg, 32768);
...
ftpxferproc(void *v)
{
	...
	vtime = 0;
	for(n = 0;;n += i){
		i = read(arg->in, buf, sizeof(buf));
		if(i == 0)
			break;
		if(i < 0) {
		err:
			sendul(arg->cwait, Error);
			return;
		}
		if(output(arg->out, buf, i) != i)
			goto err;
		r->start += i;
		if(verbose && (vtime != time(0) || r->start == r->end)) {
			vtime = time(0);
