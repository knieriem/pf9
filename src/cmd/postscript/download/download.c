 ...
 */
    ...
    exits(nil);			/* not much could be wrong */
 ...
 */
	...
	if ( (stringspace = (char *)malloc(d->length + 2)) == NULL )
	    error(FATAL, "no memory");
	if ( readn(fd, stringspace, d->length) == -1 )
	    error(FATAL, "can't read %s", path);
	close(fd);

	stringspace[d->length] = '\n';	/* just to be safe */
	stringspace[d->length+1] = '\0';
	free(d);
